...
const solve = (round, shape) => flow(
  lines,
  map(s => round(s) + shape(s)),
  concatAll(N.SemigroupSum)
);

assert(15 === solve(round1, shape1)(example));
assert(12 === solve(round2, shape2)(example));

log("Solution day 2, part 1: " + solve(round1, shape1)(file))();
log("Solution day 2, part 2: " + solve(round2, shape2)(file))();
